...
public class MapHandler {
	...
	public void loadMaps(GameWorld world) throws Exception {
		...
		while (resultSet.next()) {
			...
			map.setCanAuction(("0".equals(resultSet
					.getString("auction_enabled")) ? false : true));
			map.setCanPVP(("0".equals(resultSet.getString("pvp_enabled")) ? false
					: true));
			map.setCanChat(("0".equals(resultSet.getString("chat_enabled")) ? false
					: true));
			map.setCanShout(("0".equals(resultSet.getString("shout_enabled")) ? false
					: true));
			map.setCanUseItems(("0".equals(resultSet.getString("items_enabled")) ? false
					: true));
			map.setCanCast(("0".equals(resultSet.getString("spells_enabled")) ? false
					: true));
			map.setCanBind(("0".equals(resultSet.getString("bind_enabled")) ? false
					: true));
			map.setCanSpawnPets(("0".equals(resultSet.getString("pets_enabled")) ? false
					: true));
			this.maps.add(map);
		}
		resultSet.close();
		for (Map map : this.maps) {
			...
			ev.setTicks(ev.getTicks() + world.getTimerFrequency()
					* GameSettings.getDefault().getItemGroundSweepTime());
			ev.setData(map);
