...
public class Guild {
  ...
  public GuildRanks getRank(Goose.Player player) throws Exception {
    ...
    return ((PlayerGuildStatus) this.getMembers().get(player.getPlayerID())).getRank();
  }
  ...
  public void save(GameWorld world) throws Exception {
    ...
    if (this.getID() == 0) {
      ...
      ResultSet resultSet =
          world.getSqlConnection().createStatement().executeQuery("SELECT @@IDENTITY");
      resultSet.next();
      // TODO: Have to check.
    ...
    for (Object __dummyForeachVar2 : this.getMembers().values()) {
      ...
      if (status.getDirty()) {
        if (status.getRank() == GuildRanks.Deleted) {
          query =
              "DELETE FROM guild_members WHERE guild_id=" + this.getID() + " AND player_id="
                  + status.getPlayerID();
          ...
          if (status.getJustAdded()) {
            query =
                "INSERT INTO guild_members (guild_id, player_id, guild_rank) VALUES ("
                    + this.getID() + ", " + status.getPlayerID() + ", "
                    + status.getRank().ordinal() + ")";
            world.getSqlConnection().createStatement().executeQuery(query);
            status.setJustAdded(false);
          } else {
            query =
                "UPDATE guild_members SET guild_rank=" + status.getRank().ordinal()
                    + " WHERE guild_id=" + this.getID() + " AND player_id=" + status.getPlayerID();
            world.getSqlConnection().createStatement().executeQuery(query);
          }
        }
  ...
  public void changeOwner(Goose.Player leader, Goose.Player newleader, GameWorld world)
      throws Exception {
    ((PlayerGuildStatus) this.getMembers().get(leader.getPlayerID())).setRank(GuildRanks.Member);
    ((PlayerGuildStatus) this.getMembers().get(leader.getPlayerID())).setDirty(true);
    ((PlayerGuildStatus) this.getMembers().get(newleader.getPlayerID())).setRank(GuildRanks.Leader);
    ((PlayerGuildStatus) this.getMembers().get(newleader.getPlayerID())).setDirty(true);
    this.setDirty(true);
  ...
  public void changeRank(Goose.Player player, GuildRanks rank, GameWorld world) throws Exception {
    ((PlayerGuildStatus) this.getMembers().get(player.getPlayerID())).setRank(rank);
    ((PlayerGuildStatus) this.getMembers().get(player.getPlayerID())).setDirty(true);
    switch (rank) {
      case Officer:
