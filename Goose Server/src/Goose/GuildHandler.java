...
public class GuildHandler   
{
    ...
    public void loadGuilds(GameWorld world) throws Exception {
        ResultSet resultSet = world.getSqlConnection().createStatement().executeQuery("SELECT * FROM guilds");
        ...
        for (Object __dummyForeachVar0 : this.guilds.values())
        {
            ...
            world.getSqlConnection().createStatement().executeQuery("SELECT * FROM guild_members WHERE guild_id=" + guild.getID());
            while (resultSet.next())
            {
    ...
    public void addSaveEvent(GameWorld world) throws Exception {
        ...
        ev.setTicks(ev.getTicks() + (long)(GameSettings.getDefault().getGuildSavePeriod() * world.getTimerFrequency()));
