...
public class Inventory {
  ...
  public void save(GameWorld world) throws Exception {
    ...
    for (int i = 1; i <= GameSettings.getDefault().getInventorySize(); i++) {
      ...
      if (slot == null) {
        query =
            "DELETE FROM inventory WHERE player_id=" + this.player.getPlayerID() + " AND slot=" + i;
      } else {
        ...
        query =
            "INSERT INTO inventory (player_id, slot, item_id, stack) VALUES ("
                + this.player.getPlayerID() + ", " + i + ", " + slot.getItem().getItemID() + ", "
                + slot.getStack() + ");";
      }
      world.getSqlConnection().createStatement().executeUpdate(query);
    }
    for (int i = 1; i <= GameSettings.getDefault().getEquippedSize(); i++) {
      ...
      if (slot == null) {
        query =
            "DELETE FROM equipped WHERE player_id=" + this.player.getPlayerID() + " AND slot=" + i;
      } else {
        if (slot.getItem().getUnsaved()) slot.getItem().addItem(world);
        ...
        world.getSqlConnection().createStatement().executeUpdate(query);
        query =
            "INSERT INTO equipped (player_id, slot, item_id) VALUES (" + this.player.getPlayerID()
                + ", " + i + ", " + slot.getItem().getItemID() + ");";
      }
      world.getSqlConnection().createStatement().executeUpdate(query);
    }
    for (int i = 1; i <= GameSettings.getDefault().getCombineBagSize(); i++) {
      ...
      if (slot == null) {
        query =
            "DELETE FROM combinebag WHERE player_id=" + this.player.getPlayerID() + " AND slot="
                + i;
      } else {
        if (slot.getItem().getUnsaved()) slot.getItem().addItem(world);
        ...
        world.getSqlConnection().createStatement().executeUpdate(query);
        query =
            "INSERT INTO combinebag (player_id, slot, item_id, stack) VALUES ("
                + this.player.getPlayerID() + ", " + i + ", " + slot.getItem().getItemID() + ", "
                + slot.getStack() + ");";
      }
      world.getSqlConnection().createStatement().executeUpdate(query);
  ...
  public void combine(GameWorld world) throws Exception {
    ...
    if (match.getMinLevel() > this.player.getLevel()) {
      world.send(this.player, "$7You need to be level " + match.getMinLevel() + " to create "
          + match.getName() + ".");
      return;
    } else if (match.getMaxLevel() > 0 && match.getMaxLevel() < this.player.getLevel()) {
      world.send(this.player, "$7You need to be less than level " + match.getMaxLevel()
          + " to create " + match.getName() + ".");
      return;
    } else if (match.getMinExperience() > this.player.getExperience()
        + this.player.getExperienceSold()) {
      world.send(this.player, "$7You need " + match.getMinExperience() + " experience to create "
          + match.getName() + ".");
      return;
    } else if (match.getMaxExperience() > 0
        && match.getMaxExperience() < this.player.getExperience() + this.player.getExperienceSold()) {
      world.send(this.player, "$7You need less than " + match.getMaxExperience()
          + " experience to create " + match.getName() + ".");
      return;
    } else if ((match.getClassRestrictions() & (long) Math.round(Math.pow(2.0, (double) this.player
