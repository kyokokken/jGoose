...
public class ItemHandler {
	 ...
	 * 
	 */
	public void loadTemplates(GameWorld world) throws Exception {
		...
		while (resultSet.next()) {
			...
			template.setType(Goose.ItemTemplate.ItemTypes.values()[resultSet
					.getInt("item_type")]);
			template.setSlot(Goose.ItemTemplate.ItemSlots.values()[resultSet
					.getInt("item_slot")]);
			template.setUseType(Goose.ItemTemplate.UseTypes.values()[resultSet
					.getInt("item_usetype")]);
			...
			template.getBaseStats().setIntelligence(
					resultSet.getInt("stat_int"));
			...
			template.getBaseStats().setEarthResist(
					resultSet.getInt("res_earth"));
			template.getBaseStats().setSpiritResist(
					resultSet.getInt("res_spirit"));
			template.getBaseStats().setWaterResist(
					resultSet.getInt("res_water"));
			...
			template.setClassRestrictions(resultSet
					.getLong("class_restrictions"));
			template.setIsLore(("0".equals(resultSet.getString("lore")) ? false
					: true));
			template.setIsBindOnPickup(("0".equals(resultSet
					.getString("bindonpickup")) ? false : true));
			template.setIsBindOnEquip(("0".equals(resultSet
					.getString("bindonequip")) ? false : true));
			template.setIsEvent(("0".equals(resultSet.getString("event")) ? false
					: true));
			...
			template.setSpellEffect(world.getSpellHandler().getSpellEffect(
					template.getSpellEffectID()));
			if (template.getSpellEffectID() != 0
					&& template.getSpellEffect() == null) {
				continue;
			}

			// log bad spell effect on item
			template.setSpellEffectChance(Double.parseDouble(resultSet
					.getString("spell_effect_chance")));
	 ...
	 * 
	 */
	public void loadItems(GameWorld world) throws Exception {
		...
		while (resultSet.next()) {
			...
			item.setDirty(false);
			item.setIsBound(("0".equals(resultSet.getString("bound")) ? false
					: true));
	 ...
	 * 
	 * What it does is adds the item to the newitems list, which temporarily
	 * holds the item until the ItemHandler is called to save the items to
	 * database. So once it has an ID it gets put into the items hashtable.
	 * 
	 */
	...
	public Item getGold() throws Exception {
		return (Item) this.items.get(GameSettings.getDefault()
				.getItemIDStartpoint()
	...
	public void save(GameWorld world) throws Exception {
		...
		for (Object __dummyForeachVar0 : this.items.values()) {
			Item item = (Item) __dummyForeachVar0;
			if (item.getDelete()) {
	...
	public void addSaveEvent(GameWorld world) throws Exception {
		...
		ev.setTicks(ev.getTicks()
				+ (long) (GameSettings.getDefault().getItemSavePeriod() * world
						.getTimerFrequency()));
