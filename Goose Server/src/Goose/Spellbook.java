...
public class Spellbook {
  ...
  public void save(GameWorld world) throws Exception {
    ...
    for (int i = 1; i <= GameSettings.getDefault().getSpellbookSize(); i++) {
      ...
      if (slot == null) {
        query =
            "DELETE FROM spellbook WHERE player_id=" + this.player.getPlayerID() + " AND slot=" + i;
      } else {
        query =
            "DELETE FROM spellbook WHERE player_id=" + this.player.getPlayerID() + " AND slot=" + i
                + ";";
        ...
        query =
            "INSERT INTO spellbook (player_id, slot, spell_id, last_casted) VALUES ("
                + this.player.getPlayerID() + ", " + i + ", " + slot.getID() + ", "
