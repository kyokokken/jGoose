...
public class NPC implements ICharacter {
	...
	public void setAggroTargetToValue(
			HashMap<Goose.Player, Goose.NPC.Aggro> value) {
	...
	public String mKCString() throws Exception {
		return "MKC"
				...
				+ "," + this.getEquippedItems() + "," + this.getHairR() + ","
	...
	public void moveTo(GameWorld world, int x, int y) throws Exception {
		...
		if (this.getAggroTarget() != null) {
			...
			for (Goose.NPC npc : npcs) {
				if (!this.getAllies().contains(npc.getNPCTemplate()))
					continue;

				if (npc.getAggroTarget() != null)
					continue;

				if (Math.abs(npc.getMapX() - this.getMapX()) <= npc
						.getAggroRange()
						&& Math.abs(npc.getMapY() - this.getMapY()) <= npc
								.getAggroRange()) {
			...
			// TrimEnd() removes only the last occurrence of the char/string.
			if (packet.contains("\u0001"))
				packet = packet.substring(packet.lastIndexOf("\u0001"));
	 ...
	 * 
	 */
	public boolean loadFromTemplate(GameWorld world, int map_id, int map_x,
			int map_y, NPCTemplate template) throws Exception {
		...
		this.setBaseStats(AttributeSet.add(this.getBaseStats(),
	...
	public void addMoveEvent(GameWorld world) throws Exception {
		if (this.getMoveEvent() == null && this.getMoveSpeed() > 0.0) {
			...
			ev.setTicks(ev.getTicks()
					+ (long) ((this.getMoveSpeed() * (1 + snared)) * world
							.getTimerFrequency()));
			ev.setNPC(this);
	...
	public int nextStepTo(int x, int y, GameWorld world) throws Exception {
		...
		int shortestpath = Math.abs(x - (this.getMapX()))
				+ Math.abs(y - (this.getMapY()));
		int shortest = 0;
		int temp;
		...
		int rand = 0;
		if (shortestpath == Math.abs(x - (this.getMapX()))
				+ Math.abs(y - (this.getMapY()))) {
	...
	public void addRegenEvent(GameWorld world) throws Exception {
		...
		if (this.getState() == Goose.NPC.States.Dead)
			return;
	...
	/**
             ...
             * 
             */
	public void addAggro(Goose.Player player, long value, GameWorld world)
			throws Exception {
	 ...
	 * 
	 */
	public void addAggro(Goose.Player player, long value, boolean wastaunt,
		...
		if (this.getAggroTarget() == null) {
			...
			this.getAggroTargetToValue().put(this.getAggroTarget(),
					this.getAggroValue());
		...
		if (this.getAggroTarget() == player) {
			...
			if (current != null) {
				...
				if (current.compareTo(this.getAggroValue()) > 0) {
					this.setAggroTarget(player);
					this.setAggroValue(current);
					this.getAggroTargetToValue().put(player,
							this.getAggroValue());
				} else {
					this.getAggroTargetToValue().put(player, current);
				}
	 ...
	 * 
	 */
	public void aggroIfInRange(Goose.Player player, GameWorld world)
			throws Exception {
		if (this.getAggroRange() == 0)
			return;

		if (this.getAggroTarget() != null)
			return;

		if (Math.abs(this.getMapX() - player.getMapX()) <= this.getAggroRange()
				&& Math.abs(this.getMapY() - player.getMapY()) <= this
						.getAggroRange()) {
	 ...
	 * 
	 * BattleText ids: 1 - Red text 7 - Green text 10 - STUNNED 11 - ROOTED 20 -
	 * DODGE 21 - MISS 60 - Yellow text
	 * 
	 */
	public void attacked(ICharacter character, double damage, GameWorld world)
		...
		if (character instanceof Goose.Player) {
			...
			if (this.getCurrentHP() <= 0) {
				// move off this square so null
				this.getMap()
						.setCharacter(null, this.getMapX(), this.getMapY());
				this.setState(Goose.NPC.States.Dead);
				this.setMoveEvent(null);
				this.setCurrentHP(0);
					...
					if (p.getKey().getGroup() != null) {
						if (damages.containsKey(p.getKey().getGroup())) {
							damages.put(p.getKey().getGroup(),
									(long) damages.get(p.getKey().getGroup())
											+ p.getValue().Damage);
						} else {
							damages.put(p.getKey().getGroup(),
									p.getValue().Damage);
				...
				if (highest instanceof Group) {
					...
					if (this.getLevel() + 9 < ((Goose.Player) highest)
							.getLevel()) {
						if (highest instanceof Pet) {
							((Pet) highest).getOwner().addExperience(
									(long) (this.getExperience() * 0.10),
									world, ExperienceMessage.TooHigh);
						}

						((Goose.Player) highest).addExperience(
								(long) (this.getExperience() * 0.10), world,
								ExperienceMessage.TooHigh);
					} else {
						if (highest instanceof Pet) {
							((Pet) highest).getOwner().addExperience(
									this.getExperience(), world,
	...
	public void addRespawnEvent(GameWorld world) throws Exception {
		...
		ev.setTicks(ev.getTicks()
				+ (long) (this.getRespawnTime() * world.getTimerFrequency()));
		ev.setNPC(this);
		world.getEventHandler().addEvent(ev);
	}
	...
	public void addAttackEvent(GameWorld world) throws Exception {
		...
		if (this.getAttackSpeed() <= 0.0 || this.getAttackRange() <= 0)
			return;
		...
		ev.setTicks(ev.getTicks()
				+ (long) ((this.getAttackSpeed() * (1 + snared)) * world
						.getTimerFrequency()));
		ev.setNPC(this);
	...
	public void attack(ICharacter character, GameWorld world) throws Exception {
		 ...
		 */
		double damage = this.getMaxStats().getStrength()
				+ this.getWeaponDamage() + this.getLevel()
		...
		// damage *= (double)GameSettings.Default.DamageModifier; npcs don't get
		// a modifier lol
		...
		if (character == this.getAggroTarget())
			this.setLastAttackTime(world.getTimeNow());
	 ...
	 * 
	 */
	public void dropItems(Goose.Player player, GameWorld world)
			throws Exception {
		for (NPCDropInfo dropinfo : this.getNPCTemplate().getDrops()) {
			if (world.getRandom().nextInt(1000000) + 1 <= GameSettings
					.getDefault().getDropRateModifier()
					* dropinfo.getDropRate() * 10000) {
				ItemSlot drop = new ItemSlot();
				if (dropinfo.getItemTemplate().getID() == GameSettings
						.getDefault().getGoldItemID()) {
					...
					if (dropinfo.getItemTemplate().getUseType() == Goose.ItemTemplate.UseTypes.Equipment
						...
						drop.getItem().setName(
								"Powerful "
										+ dropinfo.getItemTemplate().getName());
						drop.getItem().setStatMultiplier(2);
						drop.getItem().loadFromTemplate(
								dropinfo.getItemTemplate());
						drop.getItem().setTotalStats(
								dropinfo.getItemTemplate().getBaseStats());
						drop.getItem().setTotalStats(
								AttributeSet.multiply(drop.getItem()
										.getTotalStats(), drop.getItem()
										.getStatMultiplier()));
						drop.getItem().setTotalStats(
								AttributeSet.add(
										drop.getItem().getTotalStats(), drop
												.getItem().getBaseStats()));
					} else {
						drop.getItem().loadFromTemplate(
								dropinfo.getItemTemplate());
					}
				...
				tile.setPickupTime(world.getTimeNow()
						+ (GameSettings.getDefault().getItemProtectedTime() * world
								.getTimerFrequency()));
				this.getMap().placeItem(tile);
				// tile can stack
				ItemTile maptile = (ItemTile) this.getMap().getTile(
						tile.getX(), tile.getY());
				if (maptile != null && maptile instanceof ItemTile) {
					if (drop.getItem().getItemID() != GameSettings.getDefault()
							.getItemIDStartpoint()
					...
					maptile.getItemSlot().setStack(
							maptile.getItemSlot().getStack() + drop.getStack());
	...
	public void addBuff(Buff buff, GameWorld world) throws Exception {
		...
		for (Buff b : this.getBuffs()) {
			...
			if (buff.getSpellEffect() == b.getSpellEffect()
					|| buff.getSpellEffect().getBuffStacksOver()
							.contains(b.getSpellEffect())) {
		...
		// Add/remove stats
		this.setMaxStats(AttributeSet.add(this.getMaxStats(), buff
				.getSpellEffect().getStats()));
		this.addRegenEvent(world);
		packet = this.vCString();
		if (buff.getSpellEffect().getAnimation() != 0)
			packet += "\u0001SPP" + this.getLoginID() + ","
	...
	public void removeBuff(Buff buff, GameWorld world) throws Exception {
		...
		// Add/remove stats
		this.setMaxStats(AttributeSet.subtract(this.getMaxStats(), buff
				.getSpellEffect().getStats()));
		this.addRegenEvent(world);
	 ...
	 * 
	 */
	public void openVendorWindow(Goose.Player player, GameWorld world)
			throws Exception {
	 ...
	 * 
	 */
	public void closeVendorWindow(Window window, Goose.Player player,
			GameWorld world) throws Exception {
