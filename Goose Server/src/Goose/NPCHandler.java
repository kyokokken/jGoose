...
public class NPCHandler {
	...
	public void loadNPCTemplates(GameWorld world) throws Exception {
		ResultSet resultSet = world.getSqlConnection().createStatement()
				.executeQuery("SELECT * FROM npc_templates");
		while (resultSet.next()) {
			NPCTemplate npc = new NPCTemplate();

			npc.setNPCTemplateID(resultSet.getInt("npc_id"));
			npc.setNPCType(Goose.NPCTemplate.Types.values()[resultSet
					.getInt("npc_type")]);
			...
			npc.setAttackSpeed(Double.parseDouble(resultSet
					.getString("attack_speed")));
			npc.setMoveSpeed(Double.parseDouble(resultSet
					.getString("move_speed")));
			npc.setCanMove(("1".equals(resultSet.getString("stationary")) ? false
					: true));
			npc.setCanBeStunned(("0".equals(resultSet.getString("stunnable")) ? false
					: true));
			npc.setCanBeRooted(("0".equals(resultSet.getString("rootable")) ? false
					: true));
			npc.setCanBeSlowed(("0".equals(resultSet.getString("slowable")) ? false
					: true));
			npc.setCanBeKilled(("1".equals(resultSet.getString("invincible")) ? false
					: true));
			...
			npc.getBaseStats()
					.setHPPercentRegen(
							Double.parseDouble(resultSet
									.getString("hp_percent_regen")));
			npc.getBaseStats().setHPStaticRegen(
					resultSet.getInt("hp_static_regen"));
			npc.getBaseStats()
					.setMPPercentRegen(
							Double.parseDouble(resultSet
									.getString("mp_percent_regen")));
			npc.getBaseStats().setMPStaticRegen(
					resultSet.getInt("mp_static_regen"));
		...
		for (NPCTemplate template : this.templates) {
			String query = "SELECT * FROM npc_drops WHERE npc_template_id="
					+ template.getNPCTemplateID();
			...
			while (resultSet.next()) {
				...
				drop.setDropRate(Double.parseDouble(resultSet
						.getString("droprate")));
				...
				if (drop.getItemTemplate() != null)
					template.getDrops().add(drop);

			}
			resultSet.close();
			query = "SELECT * FROM npc_vendor_items WHERE npc_template_id="
					+ template.getNPCTemplateID();
			resultSet = world.getSqlConnection().createStatement()
					.executeQuery(query);
			if (resultSet.next()) {
				template.setVendorItems(new NPCVendorSlot[GameSettings
						.getDefault().getVendorSlotSize() + 1]);
				resultSet.previous();
				while (resultSet.next()) {
					...
					vslot.setItemTemplate(world.getItemHandler().getTemplate(
							resultSet.getInt("item_template_id")));
					vslot.setCanSeeStats(("0".equals(resultSet
							.getString("stats_visible")) ? false : true));
					if (vslot.getItemTemplate() != null
							&& vslot.getSlot() > 0
							&& vslot.getSlot() < GameSettings.getDefault()
									.getVendorSlotSize()) {
	...
	public NPCTemplate getNPCTemplate(int npc_id) throws Exception {
		for (NPCTemplate template : this.templates) {
			if (template.getNPCTemplateID() == npc_id)
				return template;
	...
	public int getNewID() throws Exception {
		for (int i = GameSettings.getDefault().getMaxPlayers() + 1; i < GameSettings
				.getDefault().getMaxNPCs(); i++) {
			if (this.idToNPC.get(i) == null)
				return i;
