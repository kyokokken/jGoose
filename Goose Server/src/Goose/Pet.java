...
/**
 * Pets yay. Internally a player object since it's easier than adding another
 * character type
 */
public class Pet extends Goose.Player {
	...
	public int getLoginID() {
		int startpoint = GameSettings.getDefault().getMaxPlayers()
				+ GameSettings.getDefault().getMaxNPCs() + 1;
	...
	public static Pet fromCharacter(Goose.NPC character) throws Exception {
		...
		pet.setMaxStats(AttributeSet.add(pet.getBaseStats()
				, pet.getClas().getLevel(pet.getLevel()).getBaseStats()));
		pet.getMaxStats().setHaste(
				pet.getMaxStats().getHaste()
						+ GameSettings.getDefault().getBaseHaste());
		pet.getMaxStats().setSpellDamage(
				pet.getMaxStats().getSpellDamage()
						+ GameSettings.getDefault().getBaseSpellDamage());
		pet.getMaxStats().setSpellCrit(
				pet.getMaxStats().getSpellCrit()
						+ GameSettings.getDefault().getBaseSpellCrit());
		pet.getMaxStats().setMeleeDamage(
				pet.getMaxStats().getMeleeDamage()
						+ GameSettings.getDefault().getBaseMeleeDamage());
		pet.getMaxStats().setMeleeCrit(
				pet.getMaxStats().getMeleeCrit()
						+ GameSettings.getDefault().getBaseMeleeCrit());
		pet.getMaxStats().setDamageReduction(
				pet.getMaxStats().getDamageReduction()
						+ GameSettings.getDefault().getBaseDamageReduction());
		pet.getMaxStats().setHPPercentRegen(
				pet.getMaxStats().getHPPercentRegen()
						+ GameSettings.getDefault().getBaseHPPercentRegen());
		pet.getMaxStats().setHPStaticRegen(
				pet.getMaxStats().getHPStaticRegen()
						+ GameSettings.getDefault().getBaseHPStaticRegen());
		pet.getMaxStats().setMPPercentRegen(
				pet.getMaxStats().getMPPercentRegen()
						+ GameSettings.getDefault().getBaseMPPercentRegen());
		pet.getMaxStats().setMPStaticRegen(
				pet.getMaxStats().getMPStaticRegen()
						+ GameSettings.getDefault().getBaseMPStaticRegen());
		pet.setCurrentHP(pet.getMaxStats().getHP());
		pet.setCurrentMP(pet.getMaxStats().getMP());
		pet.setExperience(pet.getClas().getLevel(pet.getLevel())
				.getExperience() / 2);
	...
	public static Pet fromReader(ResultSet resultSet, GameWorld world)
			throws Exception {
		Pet pet = new Pet();
		...
		pet.getMaxStats().setMPStaticRegen(
				pet.getMaxStats().getMPStaticRegen()
						+ GameSettings.getDefault().getBaseMPStaticRegen());
		pet.setClass(world.getClassHandler().getClass(pet.getClassID()));
		pet.setMaxStats(AttributeSet.add(pet.getMaxStats(), pet.getClas()
				.getLevel(pet.getLevel()).getBaseStats()));
		pet.setCurrentHP(pet.getMaxStats().getHP());
		pet.setCurrentMP(pet.getMaxStats().getMP());
	...
	public void saveToDatabase(GameWorld world) throws Exception {
		if (this.getAutoCreatedNotSaved()) {
			...
			String query = "UPDATE pets SET " + "pet_name=" + "?" + ", "
					+ "pet_title=" + "?" + ", " + "pet_surname=" + "?" + ", "
					+ "pet_facing="
					+ this.getFacing()
					+ ", "
					+ "pet_level="
...
//		Pet.LoginIDToPet.put(this.getLoginID(), null);
	...
	public void addMoveEvent(GameWorld world) throws Exception {
		if (this.getMoveEvent() == null) {
			...
			ev.setTicks(ev.getTicks()
					+ (long) (this.getMoveSpeed() * world.getTimerFrequency()));
			ev.setPlayer(this);
	...
	public void addAttackEvent(GameWorld world) throws Exception {
		if (this.getAttackEvent() == null) {
			...
			ev.setTicks(ev.getTicks()
					+ (long) (this.getAttackSpeed() * world.getTimerFrequency()));
			ev.setPlayer(this);
			world.getEventHandler().addEvent(ev);
	...
	/**
	 * WPSSTring, weapon speed string Overrides to fix a bug since pets don't
	 * have a weapondelay
	 * 
	 */
