	...
	public void ready(GameWorld world) throws Exception {
		...
		if (pet.getIsAlive()) {
			...
			if (pet.getTarget() != null
					&& (pet.getTarget().getMap() != pet.getMap() || (pet
							.getTarget() instanceof Goose.NPC && ((Goose.NPC) pet
			...
			switch (pet.getMode()) {
				case Neutral:
					direction = pet.nextStepTo(pet.getOwner().getMapX()
							+ world.getRandom().nextInt(4) - 2, pet.getOwner()
							.getMapY() + world.getRandom().nextInt(4) - 2,
							world);
					break;
				case Follow:
					direction = pet.nextStepTo(pet.getOwner().getMapX(), pet
							.getOwner().getMapY(), world);
					break;
				case Defend:
				case Attack:
					if (pet.getTarget() == null) {
						direction = pet.nextStepTo(pet.getOwner().getMapX()
								+ world.getRandom().nextInt(4) - 2, pet
								.getOwner().getMapY()
								+ world.getRandom().nextInt(4) - 2, world);
					} else {
						direction = pet.nextStepTo(pet.getTarget().getMapX(),
								pet.getTarget().getMapY(), world);
					}
					break;
