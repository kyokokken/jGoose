...
public class BuyManaCommandEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == States.Ready) {
			...
			if (this.getPlayer().getClas()
					.getLevel(this.getPlayer().getLevel()).getExperience() != 0)
				return;
			...
			this.getPlayer()
					.removeStats(this.getPlayer().getBaseStats(), world);
			double buyrate = 0;
			for (int i = 1; i <= buys; i++) {
				buyrate = ((this.getPlayer().getBaseStats().getMP() / GameSettings
						.getDefault().getIncreaseManaBuyAmount()) * (double) .2) + 1;
				if (this.getPlayer().getExperience() >= (long) (this
						.getPlayer().getClas().getManaCost() * buyrate)) {
					this.getPlayer().setExperience(
							this.getPlayer().getExperience()
									- (long) (this.getPlayer().getClas()
											.getManaCost() * buyrate));
					...
					this.getPlayer()
							.getBaseStats()
							.setMP(this.getPlayer().getBaseStats().getMP()
									+ GameSettings.getDefault()
											.getManaBuyAmount());
					bought += GameSettings.getDefault().getManaBuyAmount();
					soldexp += (long) (this.getPlayer().getClas()
							.getManaCost() * buyrate);
				} else {
					break;
				}
			...
			world.send(this.getPlayer(), "$7Bought " + bought + " mp for "
					+ soldexp + " experience.");
