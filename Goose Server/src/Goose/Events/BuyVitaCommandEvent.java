	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == States.Ready) {
			...
			for (int i = 1; i <= buys; i++) {
				buyrate = ((this.getPlayer().getBaseStats().getHP() / GameSettings
						.getDefault().getIncreaseVitaBuyAmount()) * (double) .2) + 1;
				if (this.getPlayer().getExperience() >= this.getPlayer()
						.getClas().getVitaCost()
						* buyrate) {
					...
					this.getPlayer()
							.getBaseStats()
							.setHP(this.getPlayer().getBaseStats().getHP()
			...
			world.send(this.getPlayer(), "$7Bought " + bought + " hp for "
					+ soldexp + " experience.");
