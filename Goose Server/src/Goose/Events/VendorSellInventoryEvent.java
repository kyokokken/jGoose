...
public class VendorSellInventoryEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			// log bad npc/slot
			if (npcid <= 0 || slotid <= 0
					|| slotid > GameSettings.getDefault().getInventorySize())
				return;
			...
			for (Object __dummyForeachVar0 : this.getPlayer().getWindows()) {
				...
				if (window.getType() == WindowTypes.Vendor
						&& window.getNPC().getLoginID() == npcid) {
					npc = window.getNPC();
					break;
				}
			...
			if (npc.getState() != Goose.NPC.States.Alive
					|| npc.getMap() != this.getPlayer().getMap()
			...
			// log bad stack size
			if (stack <= 0 || stack > slot.getStack())
				return;

			if (slot.getItem().getValue() == 0) {
				world.send(this.getPlayer(),
						"$7I have no interest in purchasing "
								+ slot.getItem().getName() + ".");
				return;
			}

			ItemSlot sellslot = this.getPlayer().getInventory()
					.removeItem(slot.getItem(), stack, world);
			this.getPlayer().addGold(
					sellslot.getStack() * slot.getItem().getValue() / 2, world);
			world.send(this.getPlayer(), "$7Sold "
					+ sellslot.getItem().getName()
					+ (sellslot.getStack() > 1 ? " (" + sellslot.getStack()
							+ ")" : "") + " for " + sellslot.getStack()
					* sellslot.getItem().getValue() / 2 + " gold.");
		}
