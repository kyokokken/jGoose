	...
	public void ready(GameWorld world) throws Exception {
		if (this.getNPC() != null) {
				...
				if (this.getNPC().getAggroTarget() != null) {
					this.getNPC().setCurrentHP(
							this.getNPC().getCurrentHP()
									+ (int) Math.round(this.getNPC()
											.getMaxStats().getHP()
											* this.getNPC().getMaxStats()
													.getHPPercentRegen()));
					this.getNPC().setCurrentHP(
							this.getNPC().getCurrentHP() + this.getNPC()
									.getMaxStats().getHPStaticRegen());
					this.getNPC().setCurrentMP(this.getNPC().getCurrentMP() + (int) Math.round(this.getNPC().getMaxStats().getMP()
							* this.getNPC().getMaxStats().getMPPercentRegen()));
					this.getNPC().setCurrentMP(this.getNPC().getCurrentMP() + this.getNPC().getMaxStats().getMPStaticRegen());
				} else {
					this.getNPC().setCurrentHP(this.getNPC().getCurrentHP() + (int) Math.round(
							this.getNPC().getMaxStats().getHP() * 0.10));
				}
				if (this.getNPC().getCurrentHP() <= 0)
					this.getNPC().setCurrentHP(1);

				if (this.getNPC().getCurrentMP() <= 0)
					this.getNPC().setCurrentMP(1);

				String packet = this.getNPC().vCString();
				List<Goose.Player> range = this.getNPC().getMap()
						.getPlayersInRange(this.getNPC());
			...
			if (player.getState() == Goose.Player.States.Ready) {
				...
				player.setCurrentHP(player.getCurrentHP()
						+ player.getMaxStats().getHPStaticRegen());
				player.setCurrentMP(player.getCurrentMP()
						+ (int) Math.round(player.getMaxStats().getMP()
								* player.getMaxStats().getMPPercentRegen()));
				player.setCurrentMP(player.getCurrentMP()
						+ player.getMaxStats().getMPStaticRegen());
				if (player.getCurrentHP() <= 0)
					player.setCurrentHP(1);

				if (player.getCurrentMP() <= 0)
					player.setCurrentMP(1);

				String packet = player.vCString();
				List<Goose.Player> range = player.getMap().getPlayersInRange(
						player);
