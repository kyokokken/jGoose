	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready
			...
			Goose.Player player = world.getPlayerHandler().getPlayerFromData(
					name);
			if (player == null) {
				world.send(this.getPlayer(), "$7Player " + name
						+ " doesn't exist.");
				return;
			}
			...
			player.setClass(world.getClassHandler().getClass(
					player.getClassID()));
			player.addStats(player.getClas().getLevel(player.getLevel())
					.getBaseStats(), world);
			...
			for (int level = 1; level <= player.getLevel(); level++) {
				if (level > player.getClas().getMaxLevel())
					break;

				for (Spell spell : player.getClas().getLevel(level)
						.getSpells()) {
