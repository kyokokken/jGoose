		...
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			if (itile == null) {
				...
				if (itile == null) {
					...
					if (!(tile.getOwner() == this.getPlayer() || (tile
							.getOwner().getGroup() != null && tile.getOwner()
							.getGroup().getPlayers().contains(this.getPlayer())))) {
						return;
					}
			...
			if (!(tile.getPickupTime() < world.getTimeNow() || (tile.getOwner() == this
					.getPlayer() || (tile.getOwner().getGroup() != null && tile
					.getOwner().getGroup().getPlayers()
					.contains(this.getPlayer()))))) {
				return;
			}

			// picked up gold
			if (tile.getItemSlot().getItem().getItemID() == GameSettings
					.getDefault().getItemIDStartpoint()
				...
				if (tile.getItemSlot().getItem().getIsLore()
						&& this.getPlayer().hasItem(
								tile.getItemSlot().getItem().getTemplate()
										.getID()))
					return;

				if (this.getPlayer()
						.getInventory()
						.addItem(tile.getItemSlot().getItem(),
								tile.getItemSlot().getStack(), world)) {
					this.getPlayer().getMap().removeItem(tile, world);
			...
			if (this.getPlayer().getGroup() != null) {
				this.getPlayer()
						.getGroup()
						.itemPickup(this.getPlayer(), tile.getItemSlot(), world);
			}
