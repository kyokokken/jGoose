 ...
 * 
 * Server responds: - SMNMapname - Send Map Name, I'm unsure why it sends it
 * twice or if it's even needed - DSM - Done Sending Map - MKC - See
 * Player.MKCString() for full syntax. This is sent to all players in the range
 * of the player. Including the player himself. - SUCId - Set Your Character (I
 * think) - Tells the client which character is the player
 * 
 */
public class DoneLoadingMapEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.LoadingMap) {
			...
			map.setCharacter(this.getPlayer(), this.getPlayer().getMapX(), this
					.getPlayer().getMapY());
			...
			world.send(this.getPlayer(),
					"SUC" + String.valueOf(this.getPlayer().getLoginID()));
			...
			for (Object __dummyForeachVar2 : this.getPlayer().getMap()
					.getItems()) {
			...
			if (this.getPlayer().getGroup() != null) {
				this.getPlayer().getGroup()
						.sendPartyWindow(this.getPlayer(), world);
			}
