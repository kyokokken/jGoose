...
public class VendorPurchaseInventoryEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			if (npcid <= 0 || slotid <= 0
					|| slotid > GameSettings.getDefault().getVendorSlotSize())
				return;
			...
			if (slot.getItemTemplate().getIsLore()
				...
				world.send(this.getPlayer(), "$7Can't purchase "
						+ slot.getItemTemplate().getName()
						+ " as it is LORE and you already have this item.");
				return;
			}

			if (slot.getItemTemplate().getValue() * slot.getStack() > this
					.getPlayer().getGold()) {
				world.send(this.getPlayer(), "$7Can't purchase "
						+ slot.getItemTemplate().getName()
			...
			if (this.getPlayer().getInventory()
					.addItem(item, slot.getStack(), world)) {
				this.getPlayer().removeGold(
						slot.getItemTemplate().getValue() * slot.getStack(),
						world);
				if (item.getIsBindOnPickup()) {
					item.setIsBound(true);
				}

				world.send(this.getPlayer(), "$7Purchased "
						+ slot.getItemTemplate().getName()
						+ (slot.getStack() > 1 ? " (" + slot.getStack() + ")"
								: "") + " for "
						+ slot.getItemTemplate().getValue() * slot.getStack()
