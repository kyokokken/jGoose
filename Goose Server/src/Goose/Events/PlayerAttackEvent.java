	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			for (Object __dummyForeachVar1 : this.getPlayer().getWindows()) {
				...
				if (window.getType() == WindowTypes.Vendor) {
					world.send(this.getPlayer(),
							"$7You can't attack while with a vendor.");
					return;
				}

			}
			long delay = (long) (((double) (this.getPlayer().getWeaponDelay() / 10.0) * (1 - this
					.getPlayer().getMaxStats().getHaste())) * world
					.getTimerFrequency());
			...
			if (now - this.getPlayer().getLastAttack() >= delay) {
				...
				ICharacter character = this.getPlayer().getMap()
						.getCharacterAt(x, y);
				if (character != null)
