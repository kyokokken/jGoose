...
public class NPCAttackEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		...
		if (!rooted
				&& this.getNPC().getLastAttackTime()
						+ this.getNPC().getBehaviourTimeout()
						* world.getTimerFrequency() < world.getTimeNow()) {
			NPCTemplate.BehaviourTypes __dummyScrutVar0 = this.getNPC()
					.getBehaviour();
			if (__dummyScrutVar0.equals(BehaviourTypes.TeleportAggro)) {
				boolean loseaggro = true;
				this.getNPC()
						.getAggroTarget()
						.warpTo(world, this.getNPC().getMap(),
								this.getNPC().getMapX(),
				...
				this.getNPC()
						.getMap()
						.setCharacter(null, this.getNPC().getMapX(),
								this.getNPC().getMapY());
				...
				this.getNPC().moveTo(world, this.getNPC().getMapX(),
						this.getNPC().getMapY());
		...
		if (this.getNPC().getMap() == aggro.getMap()
				&& Math.abs(this.getNPC().getMapX() - aggro.getMapX()) <= this
						.getNPC().getAttackRange()
				&& Math.abs(this.getNPC().getMapY() - aggro.getMapY()) <= this
						.getNPC().getAttackRange()) {
		...
		for (Object __dummyForeachVar1 : this.getNPC().getAggroTargetToValue()
				.keySet()) {
			...
			if (this.getNPC().getMap() == player.getMap()
					&& Math.abs(this.getNPC().getMapX() - player.getMapX()) <= this
							.getNPC().getAttackRange()
					&& Math.abs(this.getNPC().getMapY() - player.getMapY()) <= this
							.getNPC().getAttackRange()) {
