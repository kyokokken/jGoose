...
public class PetVitaCommandEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			if (match.getClas().getLevel(match.getLevel()).getExperience() != 0)
				return;
			...
			for (int i = 1; i <= buys; i++) {
				buyrate = ((match.getBaseStats().getHP() / GameSettings
						.getDefault().getIncreasePetVitaBuyCost()) * (double) .2) + 1;
