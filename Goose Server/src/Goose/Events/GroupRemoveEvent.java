 ...
 * GroupRemoveEvent
 * 
 * /groupremove - removes themselves from group /groupremove player - removes
 * player from group
 * 
 */
		...
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			if (name.equals("")) {
				if (this.getPlayer().getGroup() == null) {
					...
					this.getPlayer()
							.getGroup()
							.removePlayer(this.getPlayer(), world, false,
									this.getPlayer());
					return;
				}
			}
			...
			if (player != null) {
				...
				if (this.getPlayer().getGroup() == player.getGroup()) {
					this.getPlayer()
							.getGroup()
							.removePlayer(player, world,
									(this.getPlayer() != player),
									this.getPlayer());
				} else {
					world.send(this.getPlayer(),
							"$3Player isn't in your group.");
					return;
				}
			} else {
