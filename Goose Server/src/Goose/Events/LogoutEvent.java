...
public class LogoutEvent extends Event {
	 ...
	 * 
	 * If their state is not logged in we remove them from the player list and
	 * disconnect them, since they're not actually in the game yet
	 * 
	 * If their state is loading game we have already sent a
	 * "Player has entered world" message but they're not in the game properly
	 * yet, so we just send a "Player left the world" message, remove from
	 * player handler and disconnect them
	 * 
	 * Else they're properly in the game and we have to remove them from maps,
	 * etc
	 */
	public void ready(GameWorld world) throws Exception {
		...
		if (player == null) {
			...
			if (player.getMap() != null) {
				...
				player.getMap().setCharacter(null, player.getMapX(),
						player.getMapY());
			}
			...
			if (player.getGroup() != null) {
				player.getGroup().removePlayer(player, world, false,
						this.getPlayer());
			}
