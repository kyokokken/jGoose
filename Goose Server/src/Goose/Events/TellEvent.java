 ...
 * 
 * Called when someone types /tell name message Packet format: /tell name
 * message
 * 
 * Server responds: $6[tell to] name: message, to sender and $6[tell from]
 * sender: message, to name
 * 
 * 
 */
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			if (index != -1) {
				...
				if (message.length() > 0) {
					Goose.Player recipient = world.getPlayerHandler()
							.getPlayer(name);
					if (recipient != null
							&& recipient.getState().compareTo(
									Goose.Player.States.LoadingGame) > 0) {
						if ((recipient.getToggleSettings().getValue() & ToggleSetting.Tell.getValue()) == 0) {
							world.send(this.getPlayer(), "$6[tell to] "
									+ recipient.getName() + ": " + message);
							if (recipient.getChatFilterEnabled())
								message = world.getChatFilter().filter(message);

							world.send(recipient, "$6[tell from] "
						...
						world.send(this.getPlayer(), "$6" + name
								+ " is not online.");
					}
				}
