 ...
 * 
 * Called when someone types /who [all|guild] [name] Packet format: /who
 * [all|guild] [name]
 * 
		...
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			if (packet.equals("/who")) {
				...
				if (search.length > 1) {
					...
					} else if (search[1].equals("guild")
							&& this.getPlayer().getGuild() != null) {
						players = this.getPlayer().getGuild()
								.getOnlineMembers();
					} else {
				...
				if (player.getState() == Goose.Player.States.Ready) {
					world.send(
							...
							"#["
									...
									+ "] "
									+ (isNullOrEmpty(player.getTitle()) ? player
											.getTitle() + " "
											: "")
									+ player.getName()
									+ (isNullOrEmpty(player.getSurname()) ? " "
											+ player.getSurname() : "")
