	...
	public void ready(GameWorld world) throws Exception {
		if (this.getNPC().getState() == States.Alive) {
			...
			if (this.getNPC().getAggroTarget() == null) {
				if (this.getNPC().getMoveSpeed() <= 0.0)
					return;

				if (!this.getNPC().getCanMove()) {
					// Return to spawn point if stationary npc
					if (this.getNPC().getMapX() != this.getNPC().getSpawnX()
							|| this.getNPC().getMapY() != this.getNPC()
									.getSpawnY()) {
						direction = this.getNPC().nextStepTo(
								this.getNPC().getSpawnX(),
								this.getNPC().getSpawnY(), world);
					} else {
						return;
					}
				} else {
				...
				// aggro
				if (this.getNPC().getAggroTarget().getMap() == this.getNPC()
						.getMap()
						&& Math.abs(this.getNPC().getAggroTarget().getMapX()
								- this.getNPC().getMapX()) < Map.RANGE_X
						&& Math.abs(this.getNPC().getAggroTarget().getMapY()
								- this.getNPC().getMapY()) < Map.RANGE_Y) {
					direction = this.getNPC().nextStepTo(
							this.getNPC().getAggroTarget().getMapX(),
							this.getNPC().getAggroTarget().getMapY(), world);
				} else {
					this.getNPC().getAggroTargetToValue()
							.remove(this.getNPC().getAggroTarget());
					...
					for (Entry<Player, Aggro> p : this.getNPC().getAggroTargetToValue().entrySet()) {
						if (p.getValue().compareTo(aggro) > 0) {
							if (p.getKey().getMap() == this.getNPC().getMap()
									&& Math.abs(p.getKey().getMapX()
											- this.getNPC().getMapX()) < Map.RANGE_X
									&& Math.abs(p.getKey().getMapY()
											- this.getNPC().getMapY()) < Map.RANGE_Y) {
								aggro = p.getValue();
								highest = p.getKey();
							} else {
					...
					if (highest == null) {
						...
						direction = this.getNPC()
								.nextStepTo(
										this.getNPC().getAggroTarget()
												.getMapX(),
										this.getNPC().getAggroTarget()
												.getMapY(), world);
					}
