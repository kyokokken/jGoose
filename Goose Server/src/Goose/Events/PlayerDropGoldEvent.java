...
public class PlayerDropGoldEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			...
			if (gold > this.getPlayer().getGold())
				return;

			if (this.getPlayer().getLevel() < 10) {
				world.send(this.getPlayer(),
						"$7You need to be level 10 or higher to drop gold.");
				return;
			}
			...
			// tile can stack
			ItemTile maptile = (ItemTile) this.getPlayer().getMap()
					.getTile(tile.getX(), tile.getY());
			if (maptile != null && maptile instanceof ItemTile) {
				maptile.getItemSlot().setStack(
						maptile.getItemSlot().getStack()
								+ tile.getItemSlot().getStack());
