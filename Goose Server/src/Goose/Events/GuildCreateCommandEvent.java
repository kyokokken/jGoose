...
public class GuildCreateCommandEvent extends Event {
	...
	public void ready(GameWorld world) throws Exception {
		if (this.getPlayer().getState() == Goose.Player.States.Ready) {
			if (this.getPlayer().getGold() < GameSettings.getDefault()
					.getGuildCreationCost()) {
				world.send(this.getPlayer(), "$7You need "
						+ GameSettings.getDefault().getGuildCreationCost()
			...
			if (name.length() <= 3 || name.length() > 64) {
				world.send(this.getPlayer(),
						"$7Your guild name needs to be between 3 and 64 characters.");
				return;
			}
			...
			this.getPlayer().removeGold(
					GameSettings.getDefault().getGuildCreationCost(), world);
			this.getPlayer().setGuild(new Guild());
			this.getPlayer().getGuild().setID(0);
			this.getPlayer().getGuild()
					.setMOTD(GameSettings.getDefault().getDefaultGuildMOTD());
			this.getPlayer().getGuild().setName(name);
			this.getPlayer()
					.getGuild()
					.addMember(this.getPlayer().getPlayerID(),
							GuildRanks.Leader, true, true);
			this.getPlayer().getGuild().getOnlineMembers()
					.add(this.getPlayer());
			this.getPlayer()
					.getGuild()
					.sendToGuild(
