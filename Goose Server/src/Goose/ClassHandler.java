...
public class ClassHandler {
	...
	public void loadClasses(GameWorld world) throws Exception {
		...
		while (resultSet.next()) {
			...
			c.setACMultiplier(Double.parseDouble(resultSet
					.getString("ac_multiplier")));
		...
		resultSet = world.getSqlConnection().createStatement()
				.executeQuery("SELECT * FROM class_info");
		while (resultSet.next()) {
			...
			c.getBaseStats().setHPStaticRegen(
					resultSet.getInt("hp_static_regen"));
			c.getBaseStats()
					.setMPPercentRegen(
							Double.parseDouble(resultSet
									.getString("mp_percent_regen")));
			c.getBaseStats().setMPStaticRegen(
					resultSet.getInt("mp_static_regen"));
			c.getBaseStats().setHaste(
					Double.parseDouble(resultSet.getString("haste")));
			c.getBaseStats().setSpellDamage(
					Double.parseDouble(resultSet.getString("spell_damage")));
			c.getBaseStats().setSpellCrit(
					Double.parseDouble(resultSet.getString("spell_crit")));
			c.getBaseStats().setMeleeDamage(
					Double.parseDouble(resultSet.getString("melee_damage")));
			c.getBaseStats().setMeleeCrit(
					Double.parseDouble(resultSet.getString("melee_crit")));
			c.getBaseStats().setDamageReduction(
					Double.parseDouble(resultSet.getString("damage_reduce")));
		...
		while (resultSet.next()) {
			...
			// log bad level
			spell = world.getSpellHandler().getSpell(
