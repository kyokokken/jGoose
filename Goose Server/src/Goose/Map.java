...
public class Map {
	...
	public Map(int width, int height) throws Exception {
		...
		this.characters = new ICharacter[(this.getWidth() + 1)
				* (this.getHeight() + 1)];
	 ...
	 * 
	 */
	public List<Goose.Player> getPlayersInRange(ICharacter character)
			throws Exception {
		List<Goose.Player> result = new ArrayList<>();

		for (Goose.Player p : this.players) {
			if ((Math.abs(p.getMapX() - character.getMapX()) < RANGE_X)
					&& (Math.abs(p.getMapY() - character.getMapY()) < RANGE_Y)
					&& p != character) {
				result.add(p);
			}
	 ...
	 * 
	 */
	public List<Goose.NPC> getNPCsInRange(ICharacter character)
			throws Exception {
		/*
		 * List<NPC> range = (from p in this.npcs where Math.Abs(p.MapX -
		 * character.MapX) < RANGE_X && Math.Abs(p.MapY - character.MapY) <
		 * RANGE_Y && p != character && p.State == NPC.States.Alive select
		 * p).ToList<NPC>(); return range;
		 */
		List<Goose.NPC> result = new ArrayList<>();

		for (Goose.NPC npc : this.npcs) {
			if ((Math.abs(npc.getMapX() - character.getMapX()) < RANGE_X)
					&& (Math.abs(npc.getMapY() - character.getMapY()) < RANGE_Y)
					&& npc != character
	 ...
	 * 
	 */
	public boolean canMoveTo(ICharacter character, int x, int y)
			throws Exception {
		// invalid coordinates
		if (x < 1 || x >= this.getWidth() + 1 || y < 1
				|| y >= this.getHeight() + 1)
			return false;

		if ((Math.abs(character.getMapX() - x) == 1 && Math.abs(character
				.getMapY() - y) == 0)
				|| (Math.abs(character.getMapX() - x) == 0 && Math
						.abs(character.getMapY() - y) == 1)) {
	 ...
	 * 
	 * This method checks if the players current coordinates are valid and not
	 * blocked if they're blocked it moves the player until they can be placed
	 * 
	 */
	 ...
	 * 
	 * This method checks if the items current coordinates are valid and not
	 * blocked if they're blocked it moves the item until it can be placed
	 * 
						...
						if (x > 0 && x <= this.getWidth()) {
							...
							if ((y == oy - r || y == oy + r)
								...
								if (tile == null) {
									...
									if (((ItemTile) tile).getItemSlot()
											.canStack(item.getItemSlot())) {
										item.setX(x);
										item.setY(y);
										return true;
									}
	 ...
	 * 
	 */
	public boolean isTileBlocked(ICharacter ignore, int x, int y)
			throws Exception {
		// invalid coordinates
		if (x < 1 || x >= this.getWidth() + 1 || y < 1
				|| y >= this.getHeight() + 1)
	...
	public void loadData(GameWorld world) throws Exception {
		ResultSet resultSet = world.getSqlConnection().createStatement().executeQuery("SELECT * FROM warptiles "
		...
		resultSet.close();
		resultSet = world.getSqlConnection().createStatement().executeQuery("SELECT * FROM blockedtiles "
				+ "WHERE map_id=" + this.getID());
		...
		resultSet.close();
		resultSet = world.getSqlConnection().createStatement().executeQuery("SELECT * FROM map_required_items "
				+ "WHERE map_id=" + this.getID());
	 ...
	 * 
	 */
	public boolean playerCanJoin(Goose.Player player, GameWorld world)
			throws Exception {
		if (player.getAccess() == Goose.Player.AccessStatus.GameMaster)
			return true;
		...
		if ((this.getMinExperience() != 0)
				&& (player.getExperience() + player.getExperienceSold() < this
						.getMinExperience())) {
			return false;
		}

		if ((this.getMaxExperience() != 0)
				&& (player.getExperience() + player.getExperienceSold() > this
						.getMaxExperience())) {
			return false;
		}
