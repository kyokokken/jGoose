...
public class Map {
  ...
  public List<Goose.Player> getPlayersInRange(ICharacter character) throws Exception {
    ...
    for (Goose.Player p : this.players) {
      if ((Math.abs(p.getMapX() - character.getMapX()) < RANGE_X)
          && (Math.abs(p.getMapY() - character.getMapY()) < RANGE_Y) && p != character) {
        result.add(p);
      }
  ...
  public boolean playerCanJoin(Goose.Player player, GameWorld world) throws Exception {
    ...
    if ((this.getMinExperience() != 0)
        && (player.getExperience() + player.getExperienceSold() < this.getMinExperience())) {
      return false;
    }

    if ((this.getMaxExperience() != 0)
        && (player.getExperience() + player.getExperienceSold() > this.getMaxExperience())) {
      return false;
    }
