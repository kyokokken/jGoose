 ...
 * 
 * Each item in game is seperate Holds the original template and modified/added
 * stats
 * 
 */
public class Item implements IItem {
	...
	public int getWeaponDamage() throws Exception {
		return this.weapondamage
				+ (int) Math.ceil(this.getTemplate().getWeaponDamage()
	...
	/**
	 * These properties are read only and just pass along from the templates
	 * properties
	 * 
	 */
	...
	/**
	 * This is a bitmask Therefore only limited to about 64 classes, which
	 * should be enough. If the bit is set then that class id CAN'T use the
	 * item.
	 * 
	 */
	 ...
	 * 
	 * This is when we want to just add the templates stats to our item ie when
	 * loading the items database, eg for surname/titled items
	 * 
	 * Note: Doesn't load the value from the template as we want to keep the
	 * value as 0 if the item is custom for example
	 * 
	 */
	...
	// this.ItemID = Convert.ToInt32(new SqlCommand("SELECT @@IDENTITY",
	// world.SqlConnection).ExecuteScalar());
	/**
	 * SaveItem, updates item info in database
	 * 
	 */
	public void saveItem(GameWorld world) throws Exception {
		String query = "UPDATE items SET " + "item_template_id="
				+ this.getTemplateID() + ", " + "item_name=" + "?" + ", "
				+ "item_description=" + "?" + ", " + "player_hp="
				+ this.getBaseStats().getHP() + ", " + "player_mp="
				+ this.getBaseStats().getMP() + ", " + "player_sp="
				+ this.getBaseStats().getSP() + ", " + "stat_ac="
				...
				+ this.getBaseStats().getEarthResist() + ", "
				+ "weapon_damage=" + this.weapondamage + ", " + "item_value="
				+ this.getValue() + ", " + "graphic_tile="
				+ this.getGraphicTile() + ", " + "graphic_equip="
				+ this.getGraphicEquipped() + ", " + "graphic_r="
				+ this.getGraphicR() + ", " + "graphic_g=" + this.getGraphicG()
				+ ", " + "graphic_b=" + this.getGraphicB() + ", "
				+ "graphic_a=" + this.getGraphicA() + ", " + "stat_multiplier="
