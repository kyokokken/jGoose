...
public class Player implements ICharacter {
	...
	public enum ExperienceMessage {
		/**
		 * ExperienceMessage, used for sending to AddExperience to diaplay the
		 * right message when gaining experience
		 * 
		 */
	 ...
	 * 
	 * AutoCreatedNotSaved - When a player is autocreated they're not saved
	 * until the player save event is run
	 * 
	 */
	...
	public void setCurrentHP(int value) throws Exception {
		...
		if (this.currentHP > this.getMaxStats().getHP())
			this.currentHP = this.getMaxStats().getHP();
	...
	public void setCurrentMP(int value) throws Exception {
		currentMP = value;
		if (this.currentMP > this.getMaxStats().getMP())
			this.currentMP = this.getMaxStats().getMP();
	...
	public void setCurrentSP(int value) throws Exception {
		...
		if (this.currentSP > this.getMaxStats().getSP())
			this.currentSP = this.getMaxStats().getSP();
	 ...
	 * body pose,hair id,chest id,chest r,g,b,a,helm id,helm r,g,b,a, pants
	 * id,pants r,g,b,a,shoes id,shoes r,g,b,a, shield id,shield r,g,b,a,weapon
	 * id,weapon r,g,b,a,hair_r,hair_g,hair_b,hair_a,invis,head
	 * 
	 * character type = 1 for player, 2 for regular npc, some others for banker
	 * and vendors will find later hair id = 20ish for the normal hairs head =
	 * 70-73 for faces body pose/state = 1 for normal, 3 for staff, 4 for sword
	 * body = values 100-166 are illusions, 1 is male, 11 is female. 2/12 are
	 * naga. 3 is skeleton invis = not sure at moment
	 * 
	...
	public String mKCString() throws Exception {
		...
		ItemSlot weapon = this.getInventory()
				.getEquippedSlot(EquipSlots.Weapon);
		if (weapon != null) {
		...
		return "MKC"
				...
				+ this.getFacing()
				+ ","
				+ (int) (((float) this.getCurrentHP() / this.getMaxStats()
						.getHP()) * 100) + "," + this.getCurrentBodyID() + ","
				+ (this.getCurrentBodyID() >= 100 ? 1 : pose) + ","
				+ (this.getCurrentBodyID() >= 100 ? 0 : this.getHairID()) + ","
				+ this.getInventory().equippedDisplay() + this.getHairR() + ","
				+ this.getHairG() + "," + this.getHairB() + ","
				+ this.getHairA() + "," + "0" + ","
	...
	public void loadFromAutoCreate(String name, String password, GameWorld world)
		...
		String salt = new String(base64Salt.getBytes(),
				StandardCharsets.US_ASCII);
		MessageDigest md5 = MessageDigest.getInstance("MD5");
		byte[] data = (salt + password + GameSettings.getDefault()
...
//		System.out.println(passwordHash);
		...
		world.getPlayerHandler().setCurrentID(
				world.getPlayerHandler().getCurrentID() + 1);
		this.setName(name);
		...
		this.setExperienceSold(GameSettings.getDefault()
				.getStartingExperienceSold());
		...
		this.getBaseStats().setStrength(
				GameSettings.getDefault().getStartingStrength());
		this.getBaseStats().setStamina(
				GameSettings.getDefault().getStartingStamina());
		this.getBaseStats().setIntelligence(
				GameSettings.getDefault().getStartingIntelligence());
		this.getBaseStats().setDexterity(
				GameSettings.getDefault().getStartingDexterity());
		this.getBaseStats().setFireResist(
				GameSettings.getDefault().getStartingFireResist());
		this.getBaseStats().setAirResist(
				GameSettings.getDefault().getStartingAirResist());
		this.getBaseStats().setEarthResist(
				GameSettings.getDefault().getStartingEarthResist());
		this.getBaseStats().setSpiritResist(
				GameSettings.getDefault().getStartingSpiritResist());
		this.getBaseStats().setWaterResist(
				GameSettings.getDefault().getStartingWaterResist());
		...
		this.getMaxStats().setSpellCrit(
				GameSettings.getDefault().getBaseSpellCrit());
		this.getMaxStats().setMeleeDamage(
				GameSettings.getDefault().getBaseMeleeDamage());
		this.getMaxStats().setMeleeCrit(
				GameSettings.getDefault().getBaseMeleeCrit());
		this.getMaxStats().setDamageReduction(
				GameSettings.getDefault().getBaseDamageReduction());
		this.getMaxStats().setHPPercentRegen(
				GameSettings.getDefault().getBaseHPPercentRegen());
		this.getMaxStats().setHPStaticRegen(
				GameSettings.getDefault().getBaseHPStaticRegen());
		this.getMaxStats().setMPPercentRegen(
				GameSettings.getDefault().getBaseMPPercentRegen());
...
//				.getDefaultToggleSettings());
		...
		String[] items = GameSettings.getDefault().getStartingItems()
				.split(" ");
		if (items.length > 0) {
			for (int i = 0; i < items.length; i++) {
				try {
					int templateid = Integer.valueOf(items[i]);
					ItemTemplate template = world.getItemHandler().getTemplate(
							templateid);
					if (template == null) {
						continue;
					}
	 ...
	 * 
	 * Takes the password to hash with the salt from dataabase first to check if
	 * the password is correct.
	 * 
	 * Closes the reader
	 * 
	 * Returns a LoginEvent.LoginMessages value indicating Banned/WrongPassword
	 * or Success
	 * 
	 */
	public Goose.Events.LoginEvent.LoginMessages loadFromReaderOld(
			String password, GameWorld world, ResultSet resultSet)
			throws Exception {
...
//		this.setToggleSettings((ToggleSetting) (resultSet.getLong("toggle_settings"));
	...
	public void loadPets(GameWorld world) throws Exception {
		String query = "SELECT * FROM pets WHERE owner_id="
				+ this.getPlayerID();
		ResultSet resultSet = world.getSqlConnection().createStatement()
				.executeQuery(query);
	 ...
	 * 
	 */
	public void saveToDatabase(GameWorld world) throws Exception {
		// SqlParameter playerNameParam = new SqlParameter("@playerName",
		// SqlDbType.VarChar, 50);
		// playerNameParam.Value = this.Name;
		// System.Console.WriteLine(playerNameParam.Value);
		// SqlParameter playerTitleParam = new SqlParameter("@playerTitle",
		// SqlDbType.VarChar, 50);
		// playerTitleParam.Value = this.Title;
		// SqlParameter playerSurnameParam = new SqlParameter("@playerSurname",
		// SqlDbType.VarChar, 50);
		// playerSurnameParam.Value = this.Surname;
		if (this.getGuildID() == 0 && this.getGuild() != null)
			this.getGuild().save(world);
		...
		if (this.getAutoCreatedNotSaved()) {
			...
			String query = "UPDATE players SET " + "player_name='"
					...
					+ ", " + "bound_x=" + this.getBoundX() + ", " + "bound_y="
					+ this.getBoundY() + ", " + "player_gold=" + this.getGold()
					+ ", " + "player_level=" + this.getLevel() + ", "
					+ "experience=" + this.getExperience() + ", "
					+ "experience_sold=" + this.getExperienceSold() + ", "
					+ "player_hp=" + this.getBaseStats().getHP() + ", "
					+ "player_mp=" + this.getBaseStats().getMP() + ", "
					+ "player_sp=" + this.getBaseStats().getSP() + ", "
					+ "class_id=" + this.getClassID() + ", " + "guild_id="
					+ this.getGuildID() + ", " + "stat_ac="
					...
					+ this.getBaseStats().getIntelligence() + ", "
					+ "res_fire=" + this.getBaseStats().getFireResist() + ", "
					+ "res_water=" + this.getBaseStats().getWaterResist()
					+ ", " + "res_spirit="
					+ this.getBaseStats().getSpiritResist() + ", " + "res_air="
					+ this.getBaseStats().getAirResist() + ", " + "res_earth="
					+ this.getBaseStats().getEarthResist() + ", " + "body_id="
					+ this.getBodyID() + ", " + "face_id=" + this.getFaceID()
					+ ", " + "hair_id=" + this.getHairID() + ", " + "hair_r="
					+ this.getHairR() + ", " + "hair_g=" + this.getHairG()
					+ ", " + "hair_b=" + this.getHairB() + ", " + "hair_a="
					+ this.getHairA() + ", " + "aether_threshold="
	...
	public void moveTo(GameWorld world, int x, int y) throws Exception {
		...
		String packet = "MOC" + this.getLoginID() + "," + this.getMapX() + ","
				+ this.getMapY();
	 ...
	 * 
	 */
	public void warpTo(GameWorld world, Map map, int x, int y, boolean loseaggro)
			throws Exception {
		String erc = "ERC" + this.getLoginID();
	...
	public String sNFString() throws Exception {
		return "SNF"
				+ (this.getGuild() != null ? this.getGuild().getName() : "")
				+ "," + "" + "," + this.getClas().getClassName() + ","
				+ this.getLevel() + "," + this.getMaxStats().getHP() + ","
				+ this.getMaxStats().getMP() + "," + this.getMaxStats().getSP()
				+ "," + this.getCurrentHP() + "," + this.getCurrentMP() + ","
				+ this.getCurrentSP() + "," + this.getMaxStats().getStrength()
				+ "," + this.getMaxStats().getStamina() + ","
				+ this.getMaxStats().getIntelligence() + ","
				+ this.getMaxStats().getDexterity() + ","
				+ this.getMaxStats().getAC() + ","
				+ this.getMaxStats().getFireResist() + ","
				+ this.getMaxStats().getWaterResist() + ","
				+ this.getMaxStats().getEarthResist() + ","
				+ this.getMaxStats().getAirResist() + ","
				+ this.getMaxStats().getSpiritResist() + "," + this.getGold();
	}
	...
	public void addRegenEvent(GameWorld world) throws Exception {
		...
		ev.setTicks(ev.getTicks()
				+ (long) (GameSettings.getDefault().getRegenSpeed() * world
						.getTimerFrequency()));
		ev.setData(this);
	...
	public String tNLString() throws Exception {
		...
		if (this.getClas().getLevel(this.getLevel()).getExperience() == 0) {
			...
			if (this.getLevel() == 1)
				...
				prev = this.getClas().getLevel(this.getLevel() - 1)
						.getExperience();
			long next = this.getClas().getLevel(this.getLevel())
					.getExperience();
	...
	public void changeClass(int classid, GameWorld world) throws Exception {
		...
		this.setMaxStats(AttributeSet.subtract(this.getMaxStats(), this
				.getClas().getLevel(this.getLevel()).getBaseStats()));
		this.setLevel(GameSettings.getDefault().getStartingLevel());
		this.setExperience(GameSettings.getDefault().getStartingExperience());
		this.setExperienceSold(GameSettings.getDefault()
				.getStartingExperienceSold());
		...
		this.addStats(this.getClas().getLevel(this.getLevel()).getBaseStats(),
				world);
	...
	public boolean canUse(Item item, GameWorld world) throws Exception {
		if (this.getAccess() == Goose.Player.AccessStatus.GameMaster)
			return true;
		...
		if (item.getMaxLevel() != 0 && this.getLevel() > item.getMaxLevel()) {
			world.send(this,
					"$7You are too high level to use " + item.getName() + ".");
			return false;
		}

		if ((item.getMinExperience() != 0)
				&& (this.getExperience() + this.getExperienceSold() < item
						.getMinExperience())) {
			world.send(this,
					"$7You are too low experienced to use " + item.getName()
							+ ".");
			return false;
		}
		...
		if ((item.getMaxExperience() != 0)
				&& (this.getExperience() + this.getExperienceSold() > item
						.getMaxExperience())) {
			world.send(this,
					"$7You are too high experienced to use " + item.getName()
							+ ".");
			return false;
		}

		if ((item.getClassRestrictions() & (long) Math.round(Math.pow(2.0,
				(double) this.getClas().getClassID()))) != 0) {
			world.send(this,
					"$7You are the wrong class to use " + item.getName() + ".");
			return false;
		}
	...
	public String cHPString() throws Exception {
		...
		return "CHP" + this.getLoginID() + "," + this.getCurrentBodyID() + ","
				+ pose + "," + this.getHairID() + ","
				+ this.getInventory().equippedDisplay() + this.getHairR() + ","
				+ this.getHairG() + "," + this.getHairB() + ","
				+ this.getHairA() + "," + "0" + "," + this.getFaceID();
	}
	...
	public void addStats(AttributeSet stats, GameWorld world) throws Exception {
		...
		this.getMaxStats().setHP(
				this.getMaxStats().getHP()
						+ (stats.getStamina() * GameSettings.getDefault()
								.getStaminaToHP()));
		this.getMaxStats().setMP(
				this.getMaxStats().getMP()
						+ (stats.getIntelligence() * GameSettings.getDefault()
								.getIntelligenceToMP()));
		if (this.getCurrentHP() > this.getMaxStats().getHP()) {
	...
	public void attack(ICharacter character, GameWorld world) throws Exception {
		 ...
		 * ((double)this.WeaponDamage / 10); double absorb =
		 * (double)character.MaxStats.AC / 20;
		 */
		/*
		 * double levelfactor = (this.Level / (double)character.Level); if
		 * (levelfactor < 1) levelfactor = 1.0; double damage = ((levelfactor *
		 * 0.7 * (this.MaxStats.Strength / 10.0 + 1) * this.WeaponDamage) / 3) +
		 * this.Level/3 + world.Random.Next(1, this.Level);
		 */
		double damage = 0;
		if (this.getWeaponDamage() == 1) {
			damage = this.getMaxStats().getStrength() + 1
		...
		double absorb = (1 - ((double) (character.getMaxStats().getAC() * character
				.getClas().getACMultiplier()) / maxac));
		if (world.getRandom().nextInt(10000) + 1 <= this.getMaxStats()
				.getMeleeCrit() * 10000)
			damage *= 2;
		...
		// damage -= absorb;
		damage *= absorb;
		damage -= (double) (character.getMaxStats().getAC()
				* character.getClas().getACMultiplier() / 25);
	 ...
	 * 
	 */
	public void addExperience(long exp, GameWorld world,
			ExperienceMessage message) throws Exception {
		if (GameSettings.getDefault().getExperienceCap() > 0
			...
			world.send(this,
					"$7You have reached the experience cap. Gained 0 experience points.");
			return;
		}

		// Experience modifier for everyone under the limit
		if (!(GameSettings.getDefault().getExperienceModifierLimit() > 0 && this
				.getExperience() + this.getExperienceSold() > GameSettings
				.getDefault().getExperienceModifierLimit())) {
			exp = (long) (exp * GameSettings.getDefault()
		...
		if (this.getExperience() + exp > Integer.MAX_VALUE) {
			...
			world.send(this,
					"$7You have reached the max banked. Gained 0 experience points.");
			return;
		}
		...
		if ((this.getToggleSettings().getValue() & ToggleSetting.Experience.getValue()) == 0) {
			switch (message) {
				...
				case TooFarAway:
					world.send(this,
							"$7You were too far away to gain experience.");
					break;
				case TooHigh:
					world.send(this,
							"$7You were too experienced, you only gained "
									+ exp + " experience points.");
					break;
				case TooLow:
					world.send(this,
							"$7Group members too high to gain any experience.");
					break;
		...
		while (this.getClas().getLevel(i) != null) {
			...
			if (this.getExperience() >= levelup) {
				levels++;
				if (this.getClas().getLevel(i + 1) != null
						&& this.getClas().getLevel(i + 1).getSpells().size() > 0) {
					for (Spell spell : this.getClas().getLevel(i + 1)
							.getSpells()) {
						this.learnSpell(spell.getID(), world);
					}
				}
		...
		this.removeStats(this.getClas().getLevel(this.getLevel())
				.getBaseStats(), world);
		...
		String packet = "BT" + this.getLoginID() + ",60,Level Up!,"
				+ this.getName();
	...
	public void attacked(ICharacter character, double damage, GameWorld world)
		...
		String packet;
		if (damage == 0
				|| this.getAccess() == Goose.Player.AccessStatus.GameMaster) {
		...
		if (damage > 0) {
			...
			packet = "BT" + this.getLoginID() + ",1," + (-dmg) + ","
					+ character.getName() + "\u0001";
		} else {
			packet = "BT" + this.getLoginID() + ",7,+" + (-dmg) + ","
					+ character.getName() + "\u0001";
		}
		...
		if (this.getCurrentHP() <= 0) {
			...
			world.sendToMap(this.getMap(), "$7" + this.getName()
					+ " was slain by " + character.getName() + ".");
			this.warpTo(world, this.getBoundMap(), this.getBoundX(),
					this.getBoundY());
			this.addRegenEvent(world);
			world.send(this, this.vCString());
			world.send(this, this.sNFString());
		...
		if (dmg > 0) {
			for (Pet pet : this.getPets()) {
				if (pet.getMode() == Pet.Modes.Defend
						&& pet.getTarget() == null) {
	...
	public String vCString() throws Exception {
		return "VC"
				...
				+ (int) (((float) this.getCurrentHP() / this.getMaxStats()
						.getHP()) * 100)
				+ ","
				+ (int) (((float) this.getCurrentMP() / this.getMaxStats()
						.getMP()) * 100);
	}
	...
	public void addSaveEvent(GameWorld world) throws Exception {
		...
		if ((world.getTimeNow() - this.getLastPing()) > ((GameSettings
			...
			ev.setPlayer(this);
			ev.setTicks(ev.getTicks()
					+ (GameSettings.getDefault().getPlayerSavePeriod() * world
							.getTimerFrequency()));
			world.getEventHandler().addEvent(ev);
		}
	}
	...
	public void castSpell(int spellslot, ICharacter target, GameWorld world)
		...
		if (now - lastcast >= (long) ((spell.getAether() / 1000.0) * world
				.getTimerFrequency())) {
			if (this.getCurrentHP() >= spell.getHPStaticCost()
				...
				this.setCurrentHP(this.getCurrentHP()
						- ((int) ((this.getCurrentHP() * (spell
								.getHPPercentCost() / (double) 100.0)))));
				this.setCurrentMP(this.getCurrentMP()
						- ((int) ((this.getCurrentMP() * (spell
								.getMPPercentCost() / (double) 100.0)))));
				this.setCurrentSP(this.getCurrentSP()
						- ((int) ((this.getCurrentSP() * (spell
								.getSPPercentCost() / (double) 100.0)))));
				if (this.getCurrentHP() <= 0)
					this.setCurrentHP(1);
				...
				if (this.getState() == Goose.Player.States.Ready) {
					...
					world.send(this, packet);
					world.send(this, this.sNFString());
					for (Goose.Player player : this.getMap().getPlayersInRange(
							this)) {
						world.send(player, packet);
					}
				}
				...
				world.send(this, packet);
				for (Goose.Player player : this.getMap()
						.getPlayersInRange(this)) {
					world.send(player, packet);
				}
			}
		} else {
			double wait = (((double) ((spell.getAether() / 1000.0) * world
	 ...
	 * 
	 */
	public void addBuff(Buff buff, GameWorld world, boolean refreshbar)
			throws Exception {
		...
		String packet;
		for (Buff b : this.getBuffs()) {
			if (buff.getSpellEffect().getBuffDoesntStackOver()
					.contains(b.getSpellEffect())) {
			...
			if (!b.getItemBuff()
					&& !buff.getItemBuff()
					&& (buff.getSpellEffect() == b.getSpellEffect() || buff
							.getSpellEffect().getBuffStacksOver()
							.contains(b.getSpellEffect()))) {
				...
				if (buff.getSpellEffect().getAnimation() != 0) {
					packet = "SPP" + this.getLoginID() + ","
							+ buff.getSpellEffect().getAnimation();
				...
				if (!("".equals(b.getSpellEffect().getOffEffectText())))
					world.send(this, "$7"
							+ buff.getSpellEffect().getOffEffectText());

				if (!("".equals((buff.getSpellEffect().getOnEffectText()))))
					world.send(this, "$7"
							+ buff.getSpellEffect().getOnEffectText());
		...
		if (buff.getSpellEffect().getDuration() > 0) {
			...
			ev.setTicks(ev.getTicks() + buff.getSpellEffect().getDuration()
		...
		if (buff.getSpellEffect().getEffectType() == EffectTypes.Tick
			...
			if (buff.getBuffExpireEvent().getTicks() - world.getTimeNow() > GameSettings
					.getDefault().getSpellEffectPeriod()
					* world.getTimerFrequency()) {
				BuffTickEvent ev = new BuffTickEvent();
				ev.setData(buff);
				ev.setPlayer(this);
				ev.setTicks(ev.getTicks()
						+ (long) (GameSettings.getDefault()
								.getSpellEffectPeriod() * world
								.getTimerFrequency()));
				world.getEventHandler().addEvent(ev);
			}
	 ...
	 * 
	 */
	public void removeBuff(Buff buff, GameWorld world, boolean refreshbar)
			throws Exception {
	...
	public void sendBuffBar(GameWorld world) throws Exception {
		...
		for (Buff buff : this.getBuffs()) {
			world.send(this, "BUF" + i + ","
					+ buff.getSpellEffect().getBuffGraphic() + ","
		...
		for (Buff b : this.getBuffs()) {
			if (b.getSpellEffect().getEffectType() == EffectTypes.OnMeleeHit) {
				if (world.getRandom().nextInt(10000) + 1 <= b.getSpellEffect()
						.getOnMeleeHitSpellChance() * 100)
					b.getSpellEffect().getOnMeleeHitSpell()
							.cast(this, hitter, world);
		...
		for (Buff b : this.getBuffs()) {
			if (b.getSpellEffect().getEffectType() == EffectTypes.OnAttack) {
				if (world.getRandom().nextInt(10000) + 1 <= b.getSpellEffect()
					...
					b.getSpellEffect().getOnMeleeAttackSpell()
							.cast(this, this, world);
	...
	public String wPSString() throws Exception {
		int wps = ((int) (((double) (this.getWeaponDelay() / 10.0)
				* (1 - this.getMaxStats().getHaste()) * 1000)));
		return "WPS" + wps;
	}
	...
	public void showStatsWindow(int id, GameWorld world) throws Exception {
		...
		window.setTitle((item instanceof ItemTemplate ? ((ItemTemplate) item)
				.getName() : ((Item) item).getName()));
